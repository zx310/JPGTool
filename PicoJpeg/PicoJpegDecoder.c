...
//------------------------------------------------------------------------------
static unsigned char pjpeg_need_memory_bytes_callback(unsigned char* pBuf, unsigned char buf_size, unsigned char* pBytes_actually_read, void* pCallback_data)
{
    unsigned int n;
    unsigned char* jpg_data = (unsigned char*)pCallback_data;
    n = min(g_nInFileSize - g_nInFileOfs, buf_size);
    memcpy(pBuf, jpg_data + g_nInFileOfs, n);
    *pBytes_actually_read = (unsigned char)(n);
    g_nInFileOfs += n;
    return 0;
}

static unsigned char pjpeg_need_file_bytes_callback(unsigned char* pBuf, unsigned char buf_size, unsigned char* pBytes_actually_read, void* pCallback_data)
{
    unsigned int n;
    FILE* fd = (FILE*)pCallback_data;
    n = min(g_nInFileSize - g_nInFileOfs, buf_size);
    *pBytes_actually_read = (unsigned char)(n);
    fread(pBuf, 1, n, fd);
    g_nInFileOfs += n;
...
unsigned char* pjpeg_decode_from_file(char* name, int* width, int* height, int* comps, pjpeg_scan_type_t* pScan_type, int reduce)
{
    if (NULL == name)
    {
        return NULL;
    ...
    *comps = 0;
    ...
    g_nInFileSize = file_size;
    g_nInFileOfs = 0;

    status = pjpeg_decode_init(&image_info, pjpeg_need_file_bytes_callback, srcFd, (unsigned char)reduce);
...
#if 1
    printf("image_info.m_comps=%d\n", image_info.m_comps);
    printf("row_pitch=%d\n", row_pitch);
    printf("decoded_width=%d\n", decoded_width);
    printf("decoded_height=%d\n", decoded_height);
    printf("image_info.m_MCUWidth = %d\n", image_info.m_MCUWidth);
    printf("image_info.m_MCUHeight = %d\n", image_info.m_MCUHeight);
#endif

    pDecodeJpgData = (unsigned char*)malloc(row_pitch * decoded_height);
    if (!pDecodeJpgData)
    {
        printf("malloc error!\n");
        fclose(srcFd);
    ...
    for (; ; )
    {
        ...
        if (reduce)
        {
            ...
            pDst_row = pDecodeJpgData + mcu_y * col_blocks_per_mcu * row_pitch + mcu_x * row_blocks_per_mcu * image_info.m_comps;
        ...
        else
        {
            ...
            pDst_row = pDecodeJpgData + (mcu_y * image_info.m_MCUHeight) * row_pitch + (mcu_x * image_info.m_MCUWidth * image_info.m_comps);
            for (y = 0; y < image_info.m_MCUHeight; y += 8)
            {
                ...
                for (x = 0; x < image_info.m_MCUWidth; x += 8)
                {
                    ...
                    const unsigned char* pSrcR = image_info.m_pMCUBufR + src_ofs;
                    const unsigned char* pSrcG = image_info.m_pMCUBufG + src_ofs;
                    const unsigned char* pSrcB = image_info.m_pMCUBufB + src_ofs;
    ...
    *comps = image_info.m_comps;
    ...
    fclose(srcFd);
    return pDecodeJpgData;
}

unsigned char* pjpeg_decode_from_memory(char* data, unsigned int size, int* width, int* height, int* comps, pjpeg_scan_type_t* pScan_type, int reduce)
{
    if (NULL == data)
    {
        return NULL;
    ...
    *comps = 0;
    ...
    status = pjpeg_decode_init(&image_info, pjpeg_need_memory_bytes_callback, pJpgData, (unsigned char)reduce);
